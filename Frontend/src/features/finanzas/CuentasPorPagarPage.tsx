...
export function CuentasPorPagarPage() {
  ...
  const cargarApoyo = useCallback(async () => {
    try {
      const [res, metodos, provs] = await Promise.all([
  ...
  const columns: DataTableColumn<CompraResponse>[] = [
    ...
    },
    {
      ...
      // estado, asi que ofrecerlo prometeria algo que no pasaria nada.
      key: 'estado',
      label: 'Estado',
      filterable: false,
    ...
    <ListPage
      ...
      stats={
        <>
          ...
          />
          <StatCard
            label="Ya pagado (de estas)"
            value={`S/ ${totalPagadoDeEstas.toFixed(2)}`}
