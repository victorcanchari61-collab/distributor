...
import { Alert, Badge, Button, Desplegable, Input, Modal } from '../../components/ui'
...
interface EntregaPedidoModalProps {
  ...
  onClose: () => void
  /** Ya se convirtió: quien lo abrió recarga su lista. */
  onHecho: () => void
}
 ...
 * menos, unas unidades sueltas, un producto que no estaba— se corrige aquí
 * la línea y se pide el motivo: la venta lleva y cobra solo lo entregado, y
 * lo que quedó corto queda registrado como novedad.
 */
export function EntregaPedidoModal({ pedido, almacenes, onClose, onHecho }: EntregaPedidoModalProps) {
  ...
  const convertir = async () => {
    if (!pedido) return
    if (!pedido.reservaStock && !almacenId) return setError('Elige el almacén.')

    for (const c of calculo) {
      if (c.excede) return setError(`${c.linea.producto}: no se puede entregar más de lo pedido.`)
      if (c.reducida && !c.entrega.motivoId) {
        return setError(`Elige el motivo por el que ${c.linea.producto} se entrega en menos.`)
      }
    }
    if (calculo.every((c) => c.entregada <= 0)) {
      return setError('No queda nada por entregar. Si el cliente no recibió nada, márcalo como "No entregado".')
    }
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1 border-t border-line pt-3 text-sm">
          ...
          <div className="flex items-center justify-between font-semibold">
            ...
            <span className="text-ink">S/ {total.toFixed(2)}</span>
          </div>
        </div>
      </div>
    </Modal>
