  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AppListaPagina<Compra>(
      ...
      fila: (context, compra) => _TarjetaCuentaPagar(
        compra: compra,
        color: color,
        onGestionarPagos: puede(ref, 'finanzas.pagar', Accion.cobrar)
            ? () => _gestionarPagos(context, ref, compra)
            : null,
      ),
    );
  }
      ...
      grupos: [
        ...
        Consumer(
          builder: (context, ref, _) {
            final proveedores = ref.watch(proveedoresCuentasPorPagarProvider);
            if (proveedores.isEmpty) return const SizedBox.shrink();

            return GrupoFiltro<String?>(
              titulo: 'Proveedor',
              valor: ref.watch(proveedorCuentasPorPagarFiltroProvider),
              opciones: [
