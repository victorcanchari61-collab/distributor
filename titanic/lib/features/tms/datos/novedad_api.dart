...
class NovedadApi {
  ...
  Future<List<MotivoNovedad>> motivos() async {
    ...
    return datos.map((e) => MotivoNovedad.desdeJson(e as Map<String, dynamic>)).toList();
  }
  ...
  Future<MotivoNovedad> crearMotivo(Map<String, dynamic> cuerpo) async => MotivoNovedad.desdeJson(
    await _api.post('/motivonovedad', cuerpo: cuerpo) as Map<String, dynamic>,
  );

  /// PUT /api/motivonovedad/{id}
  Future<MotivoNovedad> actualizarMotivo(int id, Map<String, dynamic> cuerpo) async =>
      MotivoNovedad.desdeJson(
        await _api.put('/motivonovedad/$id', cuerpo: cuerpo) as Map<String, dynamic>,
      );
  ...
  Future<List<Novedad>> novedades() async {
    final pagina =
        await _api.post('/novedad/listar', cuerpo: {'pagina': 1, 'porPagina': 200})
  ...
  Future<ResumenNovedades> resumen() async =>
      ResumenNovedades.desdeJson(await _api.get('/novedad/resumen') as Map<String, dynamic>);

  /// PATCH /api/novedad/{id}/verificar. El encargado cuenta lo que volvió.
  Future<Novedad> verificar(int id, Map<String, dynamic> cuerpo) async => Novedad.desdeJson(
    await _api.patch('/novedad/$id/verificar', cuerpo: cuerpo) as Map<String, dynamic>,
  );

  /// PATCH /api/novedad/{id}/reabrir
  Future<Novedad> reabrir(int id) async =>
      Novedad.desdeJson(await _api.patch('/novedad/$id/reabrir') as Map<String, dynamic>);
}
